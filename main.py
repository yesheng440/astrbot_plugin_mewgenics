...
class MewgenicsFinder(Star):
    ...
    # 搜索和截图方法
    async def _query_wiki_core(self, event: AstrMessageEvent, category: str, item_name: str):
        yield event.plain_result(f"正在前往 Mewgenics 图鉴的【{category}】区检索【{item_name}】，请稍候...")
        ...
        try:
            async with async_playwright() as p:
                ...
                row_locator = page.locator(f"#tableBody tr:has-text('{item_name}')").first

                if await row_locator.count() > 0:
                    await row_locator.screenshot(path=save_path)
                    yield event.image_result(save_path)
                else:
                    yield event.plain_result(f"图鉴的【{category}】里似乎没有找到包含【{item_name}】的记录喵~")

                await browser.close()

        except Exception as e:
            logger.error(f"截图插件运行失败: {e}")
            yield event.plain_result(f"查询出错啦，可能是网络超时或容器环境问题。\n错误信息: {str(e)}")
    ...
    async def query_ability(self, event: AstrMessageEvent, name: str):
        """查能力 [能力名称]"""
        async for res in self._query_wiki_core(event, "能力", name): yield res

    @filter.command("查被动")
    async def query_passive(self, event: AstrMessageEvent, name: str):
        """查被动 [被动名称]"""
        async for res in self._query_wiki_core(event, "被动", name): yield res

    @filter.command("查物品")
    async def query_item(self, event: AstrMessageEvent, name: str):
        """查物品 [物品名称]"""
        async for res in self._query_wiki_core(event, "物品", name): yield res

    @filter.command("查突变")
    async def query_mutation(self, event: AstrMessageEvent, name: str):
        """查突变 [突变名称]"""
        async for res in self._query_wiki_core(event, "突变", name): yield res

    @filter.command("查角色")
    async def query_character(self, event: AstrMessageEvent, name: str):
        """查角色 [角色名称]"""
        async for res in self._query_wiki_core(event, "角色", name): yield res
